...
public class InfinityPlayScreen extends InputAdapter implements Screen {
    private MyGame game;
    ...
    public InfinityPlayScreen(MyGame game, SpriteBatch batch, ShapeRenderer sr, BitmapFont font){
        this.game = game;
        this.batch = batch;
        this.sr = sr;
        this.font = font;
    ...
    public void show() {
        ...
        sounds = new Sound[69];
        for(int i=0; i<sounds.length; i++){  //sound files end with numbers from 2 to 38
            sounds[i] = Gdx.audio.newSound(Gdx.files.internal("soundclips/furElise-"+(i+1)+".wav"));
        }
        scrollSpeed = 900;
        ...
        for(int i=0; i<notes.size; i++) {
            ...
            if(n.getY()<-NOTE_HEIGHT){
                if(n.isClicked()) {
                    ...
                    dispose();
                    game.setScreen(new GameOverScreen(game, batch, sr, font));
                }
            }
        ...
        if(gameStarted) {
            for(int i=0; i<notes.size; i++) {
                ...
                if (n.contains(pointerIn3D.x, pointerIn3D.y)) {
                    if(!n.isClicked() && i!=0 && notes.get(i-1).isClicked()){
                        n.setClicked(true);
                        sounds[n.getId()%69].play(1);
